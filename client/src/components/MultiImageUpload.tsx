...
import { Upload, X, Image as ImageIcon, Loader2, Star, GripVertical, Plus } from "lucide-react";
...
interface MultiImageUploadProps {
  ...
  // For new trucks that don't have an ID yet
...
export function MultiImageUpload({ 
  ...
  const deleteImageMutation = trpc.truckImages.delete.useMutation({
  ...
  const setPrimaryMutation = trpc.truckImages.setPrimary.useMutation({
    onSuccess: (_, variables) => {
      onImagesChange(
        images.map((img) => ({
          ...img,
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      if (truckId) {
        // If truck exists, add to database
        ...
        // For new trucks, add to pending list
  ...
  const handleSetPrimary = (imageId: number) => {
    if (truckId) {
      setPrimaryMutation.mutate({ truckId, imageId });
    }
  };
