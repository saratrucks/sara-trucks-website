...
interface EquipmentMultiImageUploadProps {
  equipmentId?: number;
  images: EquipmentImage[];
  onImagesChange: (images: EquipmentImage[]) => void;
  pendingImages?: string[];
  onPendingImagesChange?: (urls: string[] | ((prev: string[]) => string[])) => void;
}

export function EquipmentMultiImageUpload({ 
  equipmentId, 
  images, 
  onImagesChange,
  pendingImages = [],
  onPendingImagesChange
}: EquipmentMultiImageUploadProps) {
  ...
  const deleteImageMutation = trpc.equipmentImages.delete.useMutation({
  ...
  const setPrimaryMutation = trpc.equipmentImages.setPrimary.useMutation({
    onSuccess: (_, variables) => {
      onImagesChange(
        images.map((img) => ({
          ...img,
  ...
  const handleSetPrimary = (imageId: number) => {
    if (equipmentId) {
      setPrimaryMutation.mutate({ equipmentId, imageId });
    }
  };
    ...
    <div className="space-y-4">
      ...
      >
        <input
          ...
          className="hidden"
          id="equipment-multi-image-upload"
        />
        ...
        {isUploading ? (
          ...
          <label
            htmlFor="equipment-multi-image-upload"
