...
import { Container, Plus, Edit, Trash2, LayoutDashboard, LogOut, Home, Search, Truck } from "lucide-react";
...
export default function TrailersList() {
  ...
  const { data: trailers, isLoading, refetch } = trpc.trailers.listAll.useQuery();
  const deleteMutation = trpc.trailers.delete.useMutation({
    onSuccess: () => {
      ...
      <main className="container py-8">
        <div className="space-y-6">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              <h1 className="text-3xl font-bold text-primary">{t.admin.trailers_management}</h1>
            ...
            <CardContent className="p-0">
              {isLoading ? (
                  ...
                  <table className="w-full">
                    ...
                    <tbody>
                      {filteredTrailers.map((trailer) => (
                        <tr key={trailer.id} className="border-b hover:bg-muted/50">
                          <td className="py-4 px-4 font-medium">{trailer.brand}</td>
                          <td className="py-4 px-4">{trailer.model}</td>
                          <td className="py-4 px-4">{trailer.year}</td>
                          ...
                          <td className="py-4 px-4">
                            <div className="flex gap-2">
                              <Link href={`/admin/trailers/${trailer.id}`}>
                              ...
                              <AlertDialog>
                                ...
                                <AlertDialogContent>
                                  <AlertDialogHeader>
                                    ...
                                    <AlertDialogDescription>
                                      {t.admin.delete_warning} {trailer.brand} {trailer.model}. {t.admin.cannot_undo}
                                    </AlertDialogDescription>
                                  </AlertDialogHeader>
                                  <AlertDialogFooter>
                                    ...
                                    <AlertDialogAction
                                      ...
                                      onClick={() => deleteMutation.mutate({ id: trailer.id })}
                                    >
                                      {t.admin.delete}
                                    </AlertDialogAction>
                                  </AlertDialogFooter>
                ...
                <div className="text-center py-12 text-muted-foreground">
                  ...
                  <p className="text-lg">{t.admin.no_trucks}</p>
                  <Link href="/admin/trailers/new" className="text-primary hover:underline">
                    {t.admin.forms.add_trailer}
                  </Link>
                </div>
              )}
